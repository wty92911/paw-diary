...
export interface UsePhotosActions {
  ...
  getPhotoPath: (filename: string) => Promise<string>
...
export function usePhotos(): UsePhotosState & UsePhotosActions {
  ...
  const getPhotoPath = async (filename: string): Promise<string> => {
    try {
      setError(null)
      const path = await invoke<string>('get_pet_photo_path', {
        photoFilename: filename,
      })
      return path
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get photo path'
      setError(errorMessage)
      throw new Error(errorMessage)
    }
  }
