...
impl PhotoService {
    ...
    pub fn store_photo<P: AsRef<Path>>(&self, source_path: P) -> Result<String, PetError> {
        ...
        if !target_path.exists() {
            return Err(PetError::photo_processing("Processed image file was not created"));
        }
    ...
    pub fn store_photo_from_bytes(&self, image_data: &[u8], original_extension: Option<&str>) -> Result<String, PetError> {
        // Create temporary file for processing
        let temp_filename = format!("temp_{}.{}", Uuid::new_v4(), original_extension.unwrap_or("jpg"));
    ...
    pub fn delete_photo(&self, photo_filename: &str) -> Result<(), PetError> {
        ...
        if photo_filename.contains("..") || photo_filename.contains('/') || photo_filename.contains('\\') {
    ...
    fn resize_image_with_aspect_ratio(&self, img: image::DynamicImage, target_width: u32, target_height: u32) -> image::DynamicImage {
...
mod tests {
    ...
    fn setup_test_photo_service() -> (PhotoService, TempDir) {
        ...
        let photo_service = PhotoService::new(temp_dir.path()).expect("Failed to create PhotoService");
    ...
    fn test_store_photo_from_bytes() {
        ...
        test_img.write_to(&mut std::io::Cursor::new(&mut img_bytes), ImageFormat::Jpeg).unwrap();
    ...
    fn test_delete_photo() {
        ...
        let filename = photo_service.store_photo_from_bytes(&img_bytes, Some("jpg")).unwrap();
    ...
    fn test_get_photo_info() {
        ...
        let mut img_bytes = Vec::new();
        test_img.write_to(&mut std::io::Cursor::new(&mut img_bytes), ImageFormat::Png).unwrap();
        
        let filename = photo_service.store_photo_from_bytes(&img_bytes, Some("png")).unwrap();
    ...
    fn test_list_photos() {
        ...
        test_img.write_to(&mut std::io::Cursor::new(&mut img_bytes1), ImageFormat::Jpeg).unwrap();
        
        let mut img_bytes2 = Vec::new();
        let test_img2 = create_test_image(150, 150);
        test_img2.write_to(&mut std::io::Cursor::new(&mut img_bytes2), ImageFormat::Jpeg).unwrap();
        
        photo_service.store_photo_from_bytes(&img_bytes1, Some("jpg")).unwrap();
        photo_service.store_photo_from_bytes(&img_bytes2, Some("jpg")).unwrap();
    ...
    fn test_storage_stats() {
        ...
        assert_eq!(stats.storage_dir, temp_dir.path().to_string_lossy().to_string());
        ...
        photo_service.store_photo_from_bytes(&img_bytes, Some("jpg")).unwrap();
    ...
    fn test_invalid_filename_security() {
        ...
        assert!(photo_service.get_photo_path("..\\..\\windows\\system32\\config").is_err());
    ...
    fn test_image_format_determination() {
        ...
        assert_eq!(photo_service.determine_output_format("jpg").unwrap(), ImageFormat::Jpeg);
        assert_eq!(photo_service.determine_output_format("JPEG").unwrap(), ImageFormat::Jpeg);
        assert_eq!(photo_service.determine_output_format("png").unwrap(), ImageFormat::Png);
        assert_eq!(photo_service.determine_output_format("webp").unwrap(), ImageFormat::WebP);
        
        // Unknown format should default to JPEG
        assert_eq!(photo_service.determine_output_format("unknown").unwrap(), ImageFormat::Jpeg);
    }
}